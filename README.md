...
- `#research` — **Research Studio**: a free, no-backend topic & proposal generator. The student answers a few questions (field, topic in their own words, motivating problem, methodology, keywords), then PathFinder searches **real, recent academic literature** via free, no-key, browser-callable scholarly APIs (**OpenAlex**, with **Crossref** as a resilient fallback, then a degraded offline scaffold) and produces (1) a **literature map** — top recent papers, most-active authors, trending sub-themes, a year histogram — plus **3–5 candidate research directions** and **matched NZ labs/supervisors** from the dataset; then (2) expands any chosen direction into a **full structured proposal draft** (working title, abstract, background with inline citations, research gap, research questions, methodology, indicative 3-year timeline, suggested supervisors, formatted references). Copy / download `.md`/`.txt`, auto-saved to the account (one debounced `kv` write), and a one-click "send to a mentor for review" hook. No API key, no Cloud Function, no paid services — the literature calls hit external servers, not Firestore, so it stays well inside the free Spark plan.
