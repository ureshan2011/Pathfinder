...
**Published agent performance.** INZ names education agents and their student visa approval bands for nine source markets, Sri Lanka among them, and `scripts/sync-govt-data.js` pulls the Sri Lanka sheet into `PF_GOVT.agents` (zero-dependency xlsx reader — the workbook is a ZIP of XML, and `zlib` was already in the script for PDF streams). `agentTableCard()` renders it on the Visa Hub. Reproduce, never characterise: bands stay bands, volumes stay INZ's ranges, no adjective attaches to a named firm, and the publisher's own caveats ship with the table — particularly that individual rows exclude withdrawn/lapsed applications while the market total includes them, so the two are not comparable. Narrowing a band to a point figure, or ranking firms into a verdict, turns published fact into our claim about a competitor.
