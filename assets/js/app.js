...
function renderAssessment(main) {
  ...
  main.innerHTML = viewHead('quiz', `Question ${i + 1} of ${qs.length}`, 'Pathway Assessment',
    `Seven quick questions about ${T.article} in New Zealand. Under five minutes. A roadmap built for you.`) +
...
function computeMastersResult(a) {
  ...
  if (a.degree >= 3 && a.gpa >= 3) {
    ...
    pathwayWhy = 'A four-year or honours degree with a good average meets the standard entry rule for a 180-point NZ master’s. Apply straight to the master’s — no bridging year needed.';
...
/* ── NZQA course catalogue (sharded) ──────────────────────────────────
   1,619 current postgraduate qualifications from the NZQA register, with
   the 25 universities and polytechnics that teach them, generated by
...
/* ── 2c · Courses (the NZQA postgraduate catalogue) ──────────
   The register itself, browsable: 1,619 current postgraduate
   qualifications across the 25 NZ universities and polytechnics that
   teach them, indexed by the NZQA subject-area taxonomy.
   ...
   so there is no semantic option; at 1,619 rows a plain scan is instant
...
function renderCourses(main) {
  const T = trackCfg();
  main.innerHTML = viewHead('school', 'Course Catalogue', `Find ${T.article} worth moving for`,
...
function paintCourses(main, cat) {
  ...
  main.innerHTML = viewHead('school', 'Course Catalogue',
    `${cat.meta.qualCount.toLocaleString()} postgraduate qualifications, from the source`,
    `Every current NZQF level ${T.levels.join(' and ')} qualification on the NZQA register, and the ` +
    `${cat.meta.providerCount} universities and polytechnics that teach them. ` +
...
/* ── 3 · Explore (providers, labs, supervisors) ──────────────
   ...
   they stay hand-written. What the catalogue adds is the other 17
   polytechnics (real postgraduate providers, especially for applied
   master's and PGDips), plus a live per-provider qualification count and
   the contact details that were never in the static dataset.
...
function renderExplore(main) {
  ...
  main.innerHTML = viewHead('science', isMasters() ? 'Provider Explorer' : 'Research Lab Explorer',
    ...
    isMasters()
      ? 'Every New Zealand university and polytechnic that teaches postgraduate qualifications, with what each one offers and how to reach them.'
  ...
  // Curated universities first, then the polytechnics the catalogue adds.
  const paint = field => {
...
/* The 17 polytechnics — no curated research detail, so they render as a
   compact expandable list rather than pretending to a university card. */
function polytechCards() {
  const cat = window.PF_CATALOGUE;
  if (!cat) return '';
  const rows = Object.entries(cat.providers)
    .filter(([id, p]) => p.type === 'polytechnics' && providerQualCount(id) > 0)
    .sort((a, b) => a[1].name.localeCompare(b[1].name));
  if (!rows.length) return '';

  return `<div class="sec-head" style="margin:44px 0 18px">
      <span class="tag"><span class="material-symbols-outlined" style="font-size:14px">domain</span>Institutes of technology & polytechnics</span>
      <h2 style="font-size:1.35rem;margin-top:12px">${rows.length} more providers teaching postgraduate qualifications</h2>
      <p class="muted" style="font-size:13.5px;margin-top:6px">Applied, industry-facing postgraduate diplomas and master's — often
        with lower fees and later application deadlines than the universities.</p>
    </div>` +
    rows.map(([id, p]) => {
...
/* ── 4 · Funding (scholarships + visa) ──────────────────────
   275 real scholarships from the NZQA-linked provider register, replacing
